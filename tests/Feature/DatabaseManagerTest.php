...
test('guests can render the database manager', function () {
    ...
    $response->assertSee('backdrop-blur-2xl', false);
});
...
test('guests can import a sql dump into a new database using the filename', function () {
    $manager = Mockery::mock(MySqlManager::class);
    $manager->shouldReceive('listDatabases')->times(2)->andReturn(
        [['name' => 'sandbox', 'tables' => 0, 'size_bytes' => 0, 'system' => false]],
        [['name' => 'sandbox', 'tables' => 0, 'size_bytes' => 0, 'system' => false], ['name' => 'client_dump', 'tables' => 1, 'size_bytes' => 0, 'system' => false]],
    );
    $manager->shouldReceive('listTableIndex')->times(2)->with(null)->andReturn([], [['database' => 'client_dump', 'table' => 'users', 'rows' => 1]]);
    $manager->shouldReceive('listTables')->once()->with('sandbox', null)->andReturn([]);
    $manager->shouldReceive('listTables')->once()->with('client_dump', null)->andReturn([
        ['name' => 'users', 'engine' => 'InnoDB', 'rows' => 1, 'size_bytes' => 1024, 'comment' => ''],
    ]);
    $manager->shouldReceive('getTableColumns')->once()->with('client_dump', 'users', null)->andReturn([
        ['name' => 'id', 'type' => 'bigint', 'full_type' => 'bigint unsigned', 'nullable' => false, 'default' => null, 'primary' => true, 'auto_increment' => true, 'generated' => false, 'comment' => ''],
    ]);
    $manager->shouldReceive('getForeignKeys')->once()->with('client_dump', 'users', null)->andReturn([]);
    $manager->shouldReceive('getTableRows')->once()->with('client_dump', 'users', 1, Mockery::type('int'), '', 'id', 'asc', null)->andReturn([
        'rows' => [['id' => 1]],
        'total' => 1,
    ]);
    $manager->shouldReceive('importDatabase')->once()->withArgs(function (string $database, string $path, $connection): bool {
        return $database === 'client_dump' && is_file($path) && $connection === null;
    });
    app()->instance(MySqlManager::class, $manager);

    LivewireVolt::test('dashboard')
        ->set('importDatabaseName', '')
        ->set('importFile', UploadedFile::fake()->createWithContent('client-dump.sql', 'CREATE TABLE users (id BIGINT PRIMARY KEY);'))
        ->call('importDatabase')
        ->assertSet('selectedDatabase', 'client_dump');
});
...
test('selected rows render as a discreet floating chip', function () {
    $manager = Mockery::mock(MySqlManager::class);
    $columns = [
        ['name' => 'id', 'type' => 'bigint', 'full_type' => 'bigint unsigned', 'nullable' => false, 'default' => null, 'primary' => true, 'auto_increment' => true, 'generated' => false, 'comment' => ''],
        ['name' => 'name', 'type' => 'varchar', 'full_type' => 'varchar(255)', 'nullable' => false, 'default' => null, 'primary' => false, 'auto_increment' => false, 'generated' => false, 'comment' => ''],
    ...
    LivewireVolt::test('dashboard')
        ...
        ->call('toggleRowSelection', 0)
        ->assertSee('1 selected')
        ->assertSeeHtml('data-selection-chip');
});
...
test('guests can save a forge ssh connection from the dashboard', function () {
    ...
    LivewireVolt::test('dashboard')
        ...
        ->set('connectionPrivateKeyPath', '~/.ssh/id_rsa')
